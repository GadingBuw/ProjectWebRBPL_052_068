...
<head>
    ...
    <title>Input Pesanan | DurianGo</title>
</head>
...
<body>
    ...
    $nama = $_POST['nama'];
    $email = $_POST['email'];
    $jenis = $_POST['jenisdurian'];
    $jumlah = $_POST['jumlah'];
    $alamat = $_POST['alamat'];
    $metode = $_POST['metodePembayaran'];
    
    $bukti = $_FILES['buktiPembayaran']['name'];
        $path = $_FILES['buktiPembayaran']['tmp_name'];
        $target = 'C:/xampp/htdocs/durian_go/' . basename($_FILES['buktiPembayaran']['name']);
        move_uploaded_file($path, "$target");

    $id = $_POST['id'];
    $query = mysqli_query($konek, "SELECT * from tambahproduk where judul='$jenis'");
    while ($data = mysqli_fetch_array($query)) {
        $total = $data['harga'] * $jumlah;
    }
    $query = mysqli_query($konek, "INSERT INTO pesanan VALUES('','$id','$nama','$email','$jenis','$alamat','$metode','$bukti','$jumlah','$total')") or die(mysqli_error($konek));
            ...
            <div class="editform container" style="display: flex;">
                ...
                <div class="formleft container" style="padding-left: 20px;padding-top:10px;margin-right:10px; padding-right:10px;">
                    ...
                    <table>
                        ...
                        <tr>
                            <td></td>
                            <td><button type="submit" href="keranjangsession.php" role="button" style="padding:9px; font-size:16px;margin-left: 10px; background-color: blueviolet ; border-style:none; color: black; font-family: Poppins; color:white;border-radius:10px;">Beli</button></td>
                        </tr>
                    ...
                    <h5>
                        ...
                        Durian akan segera diproses, ingin lihat pesanan anda? klik
                    </h5>
                    <h5>

                        <a href='lihatpesanan.php'> disini </a>
                    </h5>
